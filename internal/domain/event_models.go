...
type PollDBRaw struct {
	...
	PollCommentPrompts            []itx.PollCommentPromptOutput `json:"poll_comment_prompts"` // Reuse existing model
...
func (p *PollDBRaw) UnmarshalJSON(data []byte) error {
	tmp := struct {
		...
		PollCommentPrompts            []itx.PollCommentPromptOutput `json:"poll_comment_prompts"`
	...
	p.PollCommentPrompts = tmp.PollCommentPrompts
...
type VoteDBRaw struct {
	...
	CommentResponses        []itx.CommentResponse `json:"comment_responses"`
